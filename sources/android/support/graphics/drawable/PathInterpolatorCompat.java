...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f20mX = new float[min];
            this.f21mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, null);
                this.f20mX[i2] = fArr[0];
                this.f21mY[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f20mX[0])) <= 1.0E-5d && ((double) Math.abs(this.f21mY[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f20mX[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f21mY[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
                        int i5 = i4 + 1;
                        float f2 = this.f20mX[i4];
                        if (f2 >= f) {
                            this.f20mX[i] = f2;
                            i++;
                            f = f2;
            ...
            sb2.append(this.f20mX[0]);
            sb2.append(",");
            sb2.append(this.f21mY[0]);
            sb2.append(" end:");
            int i6 = min - 1;
            sb2.append(this.f20mX[i6]);
            sb2.append(",");
            sb2.append(this.f21mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f20mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f20mX[i2]) {
                length = i2;
        ...
        float f2 = this.f20mX[length] - this.f20mX[i];
        if (f2 == 0.0f) {
            return this.f21mY[i];
        }
        float f3 = (f - this.f20mX[i]) / f2;
        float f4 = this.f21mY[i];
        return f4 + (f3 * (this.f21mY[length] - f4));
    }
}
